package com.xinwei.kanfangshenqi.dropdownmenu;
...
public class DropDownMenu extends LinearLayout {
	...
	private void init(Context mContext) {
		...
		mPopupWindow.setContentView(popWindows);
		...
		mUpArrow = R.drawable.menu_arrow_up;
		mDownArrow = R.drawable.menu_arrow_down;
	...
	public void setSelectedMenuItem(int mRowSelected, int mColumnSelected) {
		...
		mMenuSelectedListener.onSelected(this, mRowSelected, mColumnSelected);
	}
	...
	private void onInit() {
		...
		mMenuList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				mMenuAdapters.get(mColumnSelected).setSelectIndex(mRowSelected);
				if (mMenuSelectedListener == null && isDebug) {
					// Toast.makeText(mContext, "MenuSelectedListener is null",
		...
		if (mMenuAdapters.size() == 0) {
			...
			if (isDebug)
				Toast.makeText(mContext,
						"If you want set Adapter by yourself,please ensure the number of adpaters equal mMenuCount",
						Toast.LENGTH_LONG).show();
			return;
		}

		for (int i = 0; i < mMenuCount; i++) {
			final RelativeLayout v = (RelativeLayout) LayoutInflater.from(mContext).inflate(R.layout.menu_item, null,
					false);
			WindowManager wm = (WindowManager) getContext().getSystemService(Context.WINDOW_SERVICE);

			int width = wm.getDefaultDisplay().getWidth();
			RelativeLayout.LayoutParams parms = new RelativeLayout.LayoutParams(width / mMenuCount,
			...
			v.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View view) {
					if(mPopupWindow!=null&&mPopupWindow.isShowing()&&idCurrentChoose==view.getId()){
						mPopupWindow.dismiss();
						return;
					}
					idCurrentChoose = view.getId();
					...
					mMenuList.startAnimation(AnimationUtils.loadAnimation(mContext, R.anim.scale_show));
//					int offset = Utils.getStatusBarHeight() + getResources().getDimensionPixelOffset(R.dimen.title_height)
//							+ getResources().getDimensionPixelOffset(R.dimen.main_menu_height);
					mPopupWindow.showAsDropDown(v);
				}
			});
		}
	}
	private void resetHintArrowStatus(){
		for (int i = 0; i < mMenuCount; i++) {
			mIvMenuArrow.get(i).setImageResource(mDownArrow);
