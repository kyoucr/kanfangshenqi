...
import com.xinwei.kanfangshenqi.view.AnimationController.OnAnimateListener;
...
public class SwitchButton extends CompoundButton {
	...
	private boolean isAnimateComplete = true;
	
	private int thumbColorFilterOn ;
	private int thumbColorFilterOff ;
	...
	public SwitchButton(Context context, AttributeSet attrs, int defStyle) {
		...
		thumbColorFilterOn = ta.getColor(R.styleable.SwitchButton_thumbColorFilterOn, 0);
		thumbColorFilterOff = ta.getColor(R.styleable.SwitchButton_thumbColorFilterOff, 0);
	...
	protected void onDraw(Canvas canvas) {
		...
		if(!isAnimateComplete){
			mConf.getThumbDrawable().setAlpha(calcAlphaThumb());
			if(isChecked()&&thumbColorFilterOff!=0)
				mConf.getThumbDrawable().setColorFilter(thumbColorFilterOff, Mode.MULTIPLY);
			else if(!isChecked()&&thumbColorFilterOn!=0)
				mConf.getThumbDrawable().setColorFilter(thumbColorFilterOn, Mode.MULTIPLY);
		}else{
			mConf.getThumbDrawable().setAlpha(255);
			mConf.getThumbDrawable().clearColorFilter();
		}
	 ...
	 * @return 0 ~ 255
	 */
	private int calcAlphaThumb() {
		int alpha = 255;
		if (mSafeZone == null || mSafeZone.right == mSafeZone.left) {

		} else {
			int backWidth = mSafeZone.right - mConf.getThumbWidth() - mSafeZone.left;
			if (backWidth > 0&&!isAnimateComplete) {
				alpha = ((mThumbZone.left - mSafeZone.left) * 255 / backWidth);
				if(alpha<=30)
					alpha = 30;
			}
